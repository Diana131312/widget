...
function formatCapacity(room: WidgetRoom): string {
  const n = room.maxCapacity ?? room.capacity;
  if (n == null || n <= 0) return "Вместимость не указана";
  return `До ${n} гостей`;
}
...
export type BanyaRoomPickerProps = {
  ...
  /** Кнопка справа в заголовке аккордеона (клик не сворачивает аккордеон) */
      ...
      >
        <AccordionItem value="rooms" className="border-b-0">
          <AccordionTrigger className="px-3 py-2.5 text-sm hover:no-underline sm:px-4">
            <h3 className="min-w-0 flex-1 text-left text-base font-semibold tracking-tight text-[#485548] sm:text-lg">
              {titleText}
            </h3>
            {headerAction ? (
              <span
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex min-h-[3rem] flex-1 flex-col gap-0.5 p-2">
                      ...
                      <p className="text-base font-semibold tabular-nums leading-snug text-[#485548]">
                        {formatCapacity(room)}
                      </p>
                    </div>
                  </button>
